 ...
 * burst of darts plays back in full and in order rather than jumping to the latest map.
 *
 * The handler is told how many events are still queued behind the one it is about to play, which is
 * how playback speeds up when the operator is throwing faster than the captures can animate.
 *
 ...
 * aborts whatever is animating, so the caller can snap straight to the derived map.
 *
 * `onIdle` fires once the queue has fully drained. It exists because the HUD must not repaint while
 * anything is still moving, and an animation cannot detect that itself: its own resolve runs while
