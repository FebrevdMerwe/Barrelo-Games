...
function postToBarrelo(message: BarreloDisplayMessage | BarreloMatchCompleteMessage): void {
  ...
  // Barrelo serves the host page and this iframe from the same origin, so pinning the target origin
  // costs nothing there and keeps display hints off any other page that might frame this one.
  //
  // dev/harness.html is necessarily a *different* origin from the Vite dev server, so that same pin
  // would silently drop every message and its "sent up by your game" panel could never show
  // anything. Relaxed in dev builds only — never in the build that ships.
...
function publish(snapshot: BarreloGameStateMessage["snapshot"], playerNames: Record<string, string>): void {
  ...
  postToBarrelo({
    ...
    // Numbers that can do nothing for the side at the board — not theirs, and touching nothing of
    // theirs. Greying them out on the manual-entry dartboard is the one hint Barrelo's own chrome
    // can give that stops a player wasting a dart on an unreachable number.
...
// is how a board ends up quietly assuming one player per side. Give each player their own group index
// (0,1,2,3) instead and the same code replays a four-side match; replay() treats both the same.
