...
function postToBarrelo(message: BarreloDisplayMessage | BarreloMatchCompleteMessage): void {
  ...
  // would silently drop every message and its "sent up by your game" panel could never show anything.
  // Relaxed in dev builds only — never in the build that ships.
...
if (import.meta.env.DEV && window.self === window.top) {
  ...
  const samplePayload: ClientGamePayload = {
    ...
    // Mirrors what the course gameMode setting merges in; without it courseFor() falls back to 9.
    options: { holes: "9" },
    // Two pairs, which is what the playerGroup setting in plugin.json lets the start screen produce.
    // Give everyone a group of their own and the same code replays a four-player solo round instead.
    playerGroups: {
      [samplePlayerIds[0]]: 0,
      [samplePlayerIds[1]]: 1,
      [samplePlayerIds[2]]: 0,
  ...
  window.setTimeout(() => {
    latestUpdate = {
      ...
      playerNames: {
        ...
        [samplePlayerIds[2]]: "Robin",
        [samplePlayerIds[3]]: "Jess",
      },
    };
    gameStateEvents.emit(GAME_STATE_EVENT, latestUpdate);
  }, 200);
