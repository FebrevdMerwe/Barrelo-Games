...
const DEFAULT_DEST = 'C:/Projects/Darts/Barrelo/external-plugins/killer';
...
// Left over from the protocol v1 out-of-process contract, which the host no longer supports.
await rm(join(dest, 'server.js'), { force: true });
...
console.log(`Deployed killer (protocolVersion ${manifest.protocolVersion}) to ${dest}`);
